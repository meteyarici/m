...
@pushOnce('scripts')
    ...
    >
        ...
        @if (core()->getConfigData('sales.checkout.mini_cart.display_mini_cart'))
            <x-shop::drawer  class="category-btn">
                <!-- Drawer Toggler -->
                <x-slot:toggle >
                    ...
                    <span class="relative" >
                        <span
                            class="fa fa-cart cursor-pointer "  style="font-size:12px"
                            role="button"
                            aria-label="@lang('shop::app.checkout.cart.mini-cart.shopping-cart')"
                            tabindex="0"
                            @click="getCart"
                        ></span>
                          <span class="teext-sm">&nbsp;&nbsp;Sepet</span>



                        @if (core()->getConfigData('sales.checkout.my_cart.summary') == 'display_item_quantity')
                            <span
                                class="absolute -top-4 rounded-[44px] bg-amber-500 px-2 py-1.5 text-xs font-semibold leading-[9px] text-white rtl:right-5 max-md:ltr:left-4 max-md:rtl:right-4"
                                v-if="cart?.items_qty"
                            >
                                @{{ cart.items_qty }}
                            </span>
                        @else
                            <span
                                class="absolute -top-4 rounded-[44px] bg-amber-500 px-2 py-1.5 text-xs font-semibold leading-[9px] text-white rtl:right-5 max-md:px-2 max-md:py-1.5 max-md:ltr:left-4 max-md:rtl:right-4"
                    ...
                    >
                        ...
                        <div class="grid flex-1 place-content-start justify-stretch gap-y-2.5">
                            <div class="flex justify-between gap-2 max-md:gap-0 max-sm:flex-wrap">
                                ...
                                <div @click="handleQtyClick($event, item)">
                                ...
                                >
                                    @lang('shop::app.checkout.cart.mini-cart.remove') x x
                                </button>
